...
vim.api.nvim_set_keymap('n', '<leader>tp', '<Cmd>!python %<CR>', { noremap = true, silent = true })
-- Map <leader>tj to run the :JCR command
vim.api.nvim_set_keymap('n', '<leader>tj', '<Cmd>JCR<CR>', { noremap = true, silent = true })
...
vim.api.nvim_create_user_command('JCR', function()
  ...
  local filepath = vim.fn.expand '%:p:h' -- Get the file's directory
  local filename = vim.fn.expand '%:t'
  local classname = vim.fn.expand '%:t:r'
  vim.cmd('cd ' .. filepath) -- Change to the file's directory
  vim.cmd('!javac ' .. filename)
  vim.cmd('split | terminal java ' .. classname)
  vim.cmd 'cd -' -- Return to the original directory
end, {})
...
require('lazy').setup({
...

  {
    ...
    -- Load themes with specific settings
    init = function()
      -- Tokyonight theme setup
      require('tokyonight').setup {
        style = 'night', -- Use the 'night' style for tokyonight
      ...
      -- Load the preferred colorscheme immediately on startup
      vim.cmd.colorscheme 'tokyonight-night'
      -- vim.cmd.colorscheme 'catppuccin-mocha'  -- Uncomment to use Catppuccin instead

      -- Configure transparency for Neovim highlights directly
      vim.api.nvim_set_hl(0, 'Normal', { bg = 'NONE' })
      vim.api.nvim_set_hl(0, 'NormalNC', { bg = 'NONE' })

      -- Additional customization if needed
      vim.cmd.hi 'Comment gui=none' -- Remove italic for comments
    end,
  },
