...
local function copy_absolute_location_reference()
...
local function copy_relative_location_reference()
  local parts = get_location_parts()
  if not parts then
    return
  end
...
local function copy_buffer_file_reference()
  local parts = get_file_parts()
  if not parts then
    return
  end
...
local function copy_buffer_directory_reference()
  local parts = get_file_parts()
  if not parts then
    return
  end
...
return {
  ...
  keys = {
    {
      '<leader>ya',
      copy_absolute_location_reference,
      mode = { 'n', 'x' },
      desc = '[Y]ank [A]bsolute ref',
    },
    {
      '<leader>yr',
      copy_relative_location_reference,
      mode = { 'n', 'x' },
      desc = '[Y]ank [R]elative ref',
    },
    {
      '<leader>yf',
      copy_buffer_file_reference,
      mode = { 'n', 'x' },
